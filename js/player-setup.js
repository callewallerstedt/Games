...
/** Online: one name + color per device before host/join. */
...
/** Local: multi-player name + color rows before pass-and-play. */
export function localSetup(g, { onBack, onStart, rulesModal }) {
    ...
    render([
      ...
      el("div", { class: "footer-actions" },
        button("Start →", { big: true, onClick: () => {
          ...
          onStart({ session, players: finalNames, playerColors: finalColors });
        } })),
    ]);
  }
  draw();
